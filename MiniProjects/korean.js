...
function LkKoreanCursorFollowing(string, querySelector, callback) {
    //Use LkKorean(string) if you don't need following the cursor!!!
    var cursorPositionStart = $(querySelector).prop('selectionStart'),
        cursorPositionEnd = $(querySelector).prop('selectionEnd'),
        firstPart = string.slice(0, cursorPositionStart),
        firstPartLength = firstPart.length;
    firstPart = LkKorean(firstPart);
    $(querySelector).val(firstPart + LkKorean(string.slice(cursorPositionStart)));
    $(querySelector).prop('selectionStart', cursorPositionStart - firstPartLength + firstPart.length);
    $(querySelector).prop('selectionEnd', cursorPositionEnd - firstPartLength + firstPart.length);
}

function LkKorean(string) {
    string = hangulToLatin(string.toLowerCase());
    string = latinToHangul(string).split("");
    string.forEach((block, index) => {
        block = hangulToLatin(block).replace(/[^a-zA-Z0-9]+/gi, "").replace(" ", "");
        if (block.startsWith("a") || block.startsWith("y") || block.startsWith("w") || block.startsWith("e") || block.startsWith("o") || block.startsWith("u") || block.startsWith("i"))
            string[index] = "0" + block;
    });
    string = latinToHangul(string.join("")).split(" ");
    return string.join(" ");
}

function latinToHangul(string) {
    var inputByOrder = Object.keys(rule).sort((a, b) => b.length - a.length || b.localeCompare(a));
    inputByOrder.forEach(latinCharacter => string = string.replace(new RegExp(latinCharacter, "gi"), rule[latinCharacter]));
    return Hangul.assemble(string);
}

function hangulToLatin(string) {
    var blocks = string.replace("0", "").split("-"),
        outputByOrder = Object.values(rule).sort((a, b) => b.length - a.length || b.localeCompare(a));
    blocks.forEach((block, index) => outputByOrder.forEach(hangulCharacter => {
        block = Hangul.disassemble(block).join("").replace(new RegExp(hangulCharacter, "gi"), rule.getKeyByValue(hangulCharacter));
        blocks[index] = block;
    }));
    return blocks.join("-");
}