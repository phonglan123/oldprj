...
function actionBox(display) {
    ...
    if (display == "hide") {
        ...
        document.getElementById("settingButton").style.width = "calc((100% / 5) - 5px)";
    }
}
...
function writePointProgress(isButton) {
    ...
    if (point >= 100000) {
        document.getElementById("masterLevelCong").innerHTML = '<br/><br/><center><svg style="width: 100px; height: 100px; border-radius: 50%; box-shadow: 0 0 16px 0 #ccc" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18"><g fill="none" fill-rule="evenodd" transform="translate(-3 -3)"><circle cx="12" cy="12" r="8" fill="#fff"></circle><path fill="#3a86ff" d="M10.9 10.9s.5.5 1.1.5c.6 0 1.1-.5 1.1-.5l3.4-3.4-3.4 6.8s-.6.8-1.1.8c-.6 0-1.1-.9-1.1-.9L7.5 7.5l3.4 3.4zm-3.4 0c1.4 1.5 4.5 6.6 4.5 7.8 0 3.2 0 0 0 0s-4.2-1.6-5.6-4.4c-.8-1.5-2.2-7 1.1-3.4zm4.5 7.8c0-1.1 4.6-7.9 4.6-7.9 3.4-3.4 2 2 1.2 3.3-1.5 2.8-5.8 4.7-5.8 4.7zm0-12.3l2.3 1.1L12 9.8 9.7 7.5 12 6.4z"></path></g></svg><br/>' + getLanguage("CongMasterLV") + "</center>";
        document.getElementById("pointBarDisplay").style.display = "none";
        document.getElementById("pointIcon").style.display = "none";
        document.getElementById("pointListDisplay").style.display = "none";
        document.getElementById("pointHr").style.display = "none";
        document.getElementById("masterLevelCong").style.display = "block";
    } else {
        document.getElementById("pointIcon").style.display = "block";
        document.getElementById("masterLevelCong").style.display = "none";
        document.getElementById("pointHr").style.display = "block";
        document.getElementById("pointBarDisplay").style.display = "block";
        document.getElementById("pointListDisplay").style.display = "block";
        document.getElementById("pointBarDisplay").innerHTML = "<b>" + getLanguage(getLevel(levelPointNeed, levelNames)) + "</b> (" + point + " " + getLanguage("point") + ")<br/>" + (getLevel(levelPointNeed, levelNames, "nextPoint")[0] - point) + getLanguage("toUpLv");
        document.getElementById("pointIcon").innerHTML = '<center><svg style="margin-top: 16px; width: 75px; height: 75px; border-radius: 50%; box-shadow: 0 0 16px 0 #ccc" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18"><g fill="none" fill-rule="evenodd" transform="translate(-3 -3)"><circle cx="12" cy="12" r="8" fill="#fff"></circle><path fill="#3a86ff" d="' + getLevelIcon(getLevel(levelPointNeed, levelNames, "index")) + '"></path></g></svg></center>';
        document.getElementById("pointListDisplay").innerHTML = writePointList(levelPointNeed, levelNames);
    }
}
...
function saveEditQuestion(id) {
    for (var question = 0; question < database.length; question++) {
        if (database[question].id == id) {
            ...
            var newContent = tinymce.get("editContent").getBody().innerHTML;
...
function showBoxEdit(id) {
    ...
    for (var question = 0; question < database.length; question++) {
        if (database[question].id == id) {
            ...
            setEditorPlaceholder(tinymce.get("editContent").getBody().innerHTML);
            ...
            tinymce.get("editContent").getBody().innerHTML = database[question].content;
...
function writeQuestion(id) {
    ...
    var content = tinymce.get("editContent").getBody();
...
function downloadOutputFile() {
    var choice = document.getElementById("outputType").value;
    if (choice == "txt") {
        var fileName = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15) + ".txt",
...
function getFileExtensions() {
    var fullPath = document.getElementById('inputFileUpload').value,
        filename;
    if (fullPath) {
...
function readTxtFile() {
    var input = $('#inputFileUpload').get(0),
...
function setEditorPlaceholder(content) {
    if (content != "")
        document.getElementById("editContent").placeholder = "";
    else if (content == "")
